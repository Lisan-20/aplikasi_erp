...
- **Library Eksternal (contoh: react-select):** Wajib menimpa (*override*) konfigurasi gaya (*styles*) bawaannya secara dinamis. **Aturan Mutlak:** 
  1. Deteksi *Dark Mode* harus dilakukan eksplisit di JS (`const isDark = document.documentElement.classList.contains('dark')`) lalu warna *hex* diinjeksi langsung (contoh: `#1e293b` untuk latar gelap, `#f8fafc` untuk teks putih) ke prop `styles` (untuk bagian `control`, `menu`, `option`, `singleValue`, `input`). Dilarang mengandalkan murni variabel CSS karena berisiko konflik pembacaan warna.
  2. Untuk mencegah *dropdown* terpotong (*clipped*) oleh *container* tabel yang memiliki *scroll* (`overflow-x-auto`), selalu gunakan props `menuPortalTarget={document.body}`, `menuPosition="fixed"`, dan set `menuPortal: base => ({ ...base, zIndex: 9999 })` di konfigurasinya.
...
### Tahap 23: Siklus Akuntansi Inti (Jurnal Otomatis) & Keuangan Terpadu
- **Master COA (Bagan Akun):** Membangun antarmuka manajemen *Chart of Accounts* (COA) untuk mendata seluruh akun keuangan perusahaan. Mengimplementasikan batasan penghapusan (*Constraint Validation*) di mana COA yang sudah dipakai di Jurnal tidak boleh dihapus.
- **Helper Jurnal Otomatis (`JurnalHelper`):** Memisahkan kompleksitas *entry* ganda (Double Entry) ke dalam *Helper* terpusat. Semua controller transaksional (POS, B2B, Pembayaran Hutang) cukup memanggil metode `buatJurnalOtomatis()` beserta array sisi Debit dan Kredit. Sistem akan otomatis memastikan *balance* dan menyimpan rekamannya.
- **Penanganan Deadlock & PDO Exception:** Mengadopsi penanganan eksepsi tingkat lanjut (`\Throwable $e`) dalam blok `DB::beginTransaction()` untuk mencegah *timeout/deadlock* pada SQL Server Octane jika terjadi kegagalan validasi *input* atau kendala asersi (*type casting*).
- **Faktur Penjualan (Siklus B2B):** Mengimplementasikan modul Kasir B2B untuk menampung tagihan yang akan dibayar mundur (Piutang). Penjualan B2B ini otomatis mencatat jurnal Debit: Piutang Usaha, Kredit: Pendapatan.
- **Pembayaran Hutang & Penerimaan Piutang:** Membangun antarmuka bagi Departemen Keuangan untuk melunasi Hutang ke Supplier dan menagih Piutang dari Klien secara parsial (cicilan) maupun penuh. Sistem otomatis mendebit/mengkredit akun Kas/Bank, menyelaraskan sisa nominal Hutang/Piutang di database, dan menyimpan log historis secara terperinci (`tc_erp_pembayaran_piutang`).
