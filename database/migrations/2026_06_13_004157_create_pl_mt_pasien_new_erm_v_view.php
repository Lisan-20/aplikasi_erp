...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.pl_mt_pasien_new_erm_v
AS
SELECT     TOP (100) PERCENT dbo.pl_tc_poli.id_pl_tc_poli, dbo.pl_tc_poli.kode_poli, dbo.pl_tc_poli.no_kunjungan, dbo.pl_tc_poli.kode_bagian, dbo.pl_tc_poli.no_antrian, dbo.pl_tc_poli.tgl_jam_poli, 
                      ...
                      MONTH(dbo.pl_tc_poli.tgl_jam_poli) AS bln, YEAR(dbo.pl_tc_poli.tgl_jam_poli) AS thn, CASE WHEN daftar_ol IS NULL THEN 1 ELSE flag_verif END AS filter_verif_ol, dbo.tc_kunjungan.status_blpl, 
                      dbo.tc_registrasi.tgl_ver, dbo.tc_registrasi.flag_ver, DAY(dbo.pl_tc_poli.tgl_jam_poli) AS Expr2, dbo.tc_registrasi.user_ver, dbo.tc_registrasi.catatan_rm, dbo.tc_registrasi.id_lengkap_erm
...
WHERE     (dbo.tc_registrasi.status_batal IS NULL) AND (dbo.tc_kunjungan.status_batal IS NULL) AND (MONTH(dbo.pl_tc_poli.tgl_jam_poli) = MONTH(GETDATE())) AND (YEAR(dbo.pl_tc_poli.tgl_jam_poli) 
                      = YEAR(GETDATE())) AND (DAY(dbo.pl_tc_poli.tgl_jam_poli) = DAY(GETDATE()))
...
");
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [pl_mt_pasien_new_erm_v]");
    }
};
