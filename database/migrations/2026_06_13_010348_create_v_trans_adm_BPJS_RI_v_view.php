...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.v_trans_adm_BPJS_RI_v
AS
...
WHERE     (dbo.tc_trans_pelayanan.flag_jurnal = 0) AND (dbo.tc_trans_kasir.status_batal IS NULL) AND (dbo.tc_trans_pelayanan.jenis_tindakan IN (2)) AND 
                      (dbo.tc_trans_kasir.seri_kuitansi IN ('AI', 'RI')) AND (dbo.tc_trans_pelayanan.kode_kelompok IN (9)) AND (dbo.tc_registrasi.kode_perusahaan = 0 OR
...
");
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [v_trans_adm_BPJS_RI_v]");
    }
};
