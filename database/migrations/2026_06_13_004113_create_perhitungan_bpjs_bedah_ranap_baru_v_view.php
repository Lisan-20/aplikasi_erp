...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.perhitungan_bpjs_bedah_ranap_baru_v
AS
...
WHERE     (dbo.tc_registrasi.tgl_jam_keluar IS NOT NULL) AND (dbo.tc_trans_pelayanan.kode_kelompok IN (8, 9, 10, 11, 12)) AND (dbo.tc_registrasi.tgl_jam_keluar BETWEEN CONVERT(DATETIME, 
                      '2017-01-01 00:00:00', 102) AND CONVERT(DATETIME, '2017-03-31 23:59:59', 102)) AND (dbo.tc_trans_pelayanan.status_batal IS NULL) AND (dbo.tc_registrasi.no_registrasi IN
                          (SELECT     no_registrasi
                            FROM          dbo.pasien_rinap_v)) AND (dbo.tc_registrasi.no_registrasi IN
                          (SELECT     no_registrasi
                            FROM          dbo.pasien_bedah_v)) AND (NOT (dbo.tc_registrasi.no_registrasi IN
                          (SELECT     no_registrasi
                            FROM          dbo.pasien_icu_v))) AND (NOT (dbo.tc_registrasi.no_registrasi IN
                          (SELECT     no_registrasi
...
");
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [perhitungan_bpjs_bedah_ranap_baru_v]");
    }
};
