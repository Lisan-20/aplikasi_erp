<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        if (Schema::hasTable('bd_tc_trans')) {
            return;
        }

        Schema::create('bd_tc_trans', function (Blueprint $table) {
            $table->increments('id_bd_tc_trans');
            $table->integer('kd_group_trans');
            $table->integer('kd_trans_bendahara')->nullable();
            $table->integer('id_bank')->nullable();
            $table->string('giro', 50)->nullable();
            $table->dateTime('tgl_bank')->nullable();
            $table->string('no_bukti', 53);
            $table->string('no_ref', 50)->nullable();
            $table->dateTime('tgl_transaksi');
            $table->string('penerima', 50)->nullable();
            $table->string('uraian')->nullable();
            $table->decimal('materai', 18)->nullable();
            $table->decimal('jumlah', 18);
            $table->integer('no_induk')->nullable();
            $table->integer('flag_modul')->nullable();
            $table->tinyInteger('flag_tmp')->nullable();
            $table->integer('flag_jurnal')->nullable();
            $table->dateTime('tgl_ver')->nullable();
            $table->string('kode_bagian', 50)->nullable();
            $table->string('acc_no', 50);
            $table->string('tx_tipe', 10);
            $table->integer('no_urut')->nullable();
            $table->integer('status')->nullable();
            $table->integer('detail')->nullable();
            $table->integer('stat_id')->nullable();
            $table->integer('id_tc_hutang_supplier_inv')->nullable();
            $table->integer('id_tc_hutang_supplier_vcr')->nullable();
            $table->integer('no_registrasi')->nullable();
            $table->integer('id_masal')->nullable();
            $table->string('no_mr', 8)->nullable();
            $table->integer('kode_tc_trans_kasir')->nullable();
            $table->integer('ko_wil')->nullable();
            $table->integer('id_trans_piutang')->nullable();
            $table->integer('id_trans_hutang')->nullable();
            $table->integer('flag_pajak')->nullable();
            $table->string('no_faktur_pajak', 50)->nullable();
            $table->decimal('jumlah_pajak', 19, 4)->nullable();
            $table->string('auto_no', 4)->nullable();

            $table->primary(['id_bd_tc_trans'], 'pk_bd_tc_trans_baru');
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('bd_tc_trans');
    }
};
