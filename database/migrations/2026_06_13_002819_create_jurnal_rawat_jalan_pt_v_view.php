...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.jurnal_rawat_jalan_pt_v
AS
SELECT     dbo.tc_trans_pelayanan.kode_tc_trans_kasir, dbo.tc_trans_pelayanan.no_registrasi, dbo.tc_trans_pelayanan.no_mr, dbo.tc_trans_pelayanan.jenis_tindakan, 
                      ...
                      dbo.tc_registrasi.kode_perusahaan, dbo.tc_registrasi.kode_kelompok, SUM(dbo.tc_trans_pelayanan.bill_rs) AS bill_rs, SUM(dbo.tc_trans_pelayanan.bill_dr1) 
                      AS bill_dr1, dbo.mt_perusahaan.flag, dbo.tc_trans_pelayanan.status_kredit
...
WHERE     (dbo.mapping_transaksi_rs_v.kode_proses = 2) AND (dbo.tc_trans_kasir.seri_kuitansi IN ('AJ', 'RJ')) AND (dbo.tc_trans_pelayanan.kode_barang IS NULL) AND 
                      (dbo.tc_trans_pelayanan.kode_perusahaan > 0) AND (dbo.mt_perusahaan.flag IS NULL OR
                      dbo.mt_perusahaan.flag = 0) AND (dbo.tc_trans_pelayanan.bill_rs > 0) AND (dbo.tc_trans_kasir.status_batal IS NULL) AND (dbo.tc_trans_pelayanan.flag_jurnal = 0) 
                      AND (dbo.tc_trans_pelayanan.status_kredit = 0 OR
                      dbo.tc_trans_pelayanan.status_kredit IS NULL) AND (dbo.mapping_transaksi_rs_v.acc_kredit > 0) AND (dbo.tc_trans_pelayanan.bill_dr1 = 0 OR
                      dbo.tc_trans_pelayanan.bill_dr1 IS NULL)
GROUP BY dbo.tc_trans_pelayanan.kode_tc_trans_kasir, dbo.tc_trans_pelayanan.no_registrasi, dbo.tc_trans_pelayanan.no_mr, dbo.tc_trans_pelayanan.jenis_tindakan, 
                      ...
                      dbo.tc_trans_pelayanan.kode_barang, dbo.mapping_transaksi_rs_v.acc_debet, dbo.mapping_transaksi_rs_v.acc_kredit, dbo.mapping_transaksi_rs_v.nama_debet, 
                      dbo.mapping_transaksi_rs_v.nama_kredit, dbo.mapping_transaksi_rs_v.nama_bagian, dbo.tc_trans_kasir.no_kuitansi, dbo.tc_trans_kasir.tgl_jam, 
                      dbo.tc_registrasi.kode_perusahaan, dbo.tc_registrasi.kode_kelompok, dbo.mt_perusahaan.flag, dbo.tc_trans_pelayanan.status_kredit
");
    }
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [jurnal_rawat_jalan_pt_v]");
    }
};
