...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.trans_pel_kasir_sum_v
AS
SELECT     SUM((CASE WHEN status_kredit = 1 THEN (- 1) ELSE 1 END) * bill_rs) AS bill_rs, SUM((CASE WHEN status_kredit = 1 THEN (- 1) ELSE 1 END) * bill_dr1) AS bill_dr1, 
                      ...
                      no_registrasi, no_mr, kode_tc_trans_kasir, status_selesai, status_batal
FROM         dbo.tc_trans_pelayanan
GROUP BY no_registrasi, no_mr, kode_tc_trans_kasir, status_selesai, status_batal
HAVING      (kode_tc_trans_kasir IS NULL) AND (status_batal IS NULL)
");
    }
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [trans_pel_kasir_sum_v]");
    }
};
