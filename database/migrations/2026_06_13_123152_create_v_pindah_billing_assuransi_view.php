...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.v_pindah_billing_assuransi
AS
SELECT     TOP (100) PERCENT dbo.mt_master_tarif.kode_tarif, dbo.mt_master_tarif.nama_tarif, dbo.mt_master_tarif.kode_bagian, dbo.mt_bagian.nama_bagian, 
                      dbo.mt_master_tarif_detail.kode_klas, dbo.mt_klas.nama_klas, (CASE WHEN dbo.mt_master_tarif_detail.bill_rs_ass IS NULL 
                      THEN 0 ELSE dbo.mt_master_tarif_detail.bill_rs_ass END) AS bill_rs, dbo.tc_trans_pelayanan.bill_rs_jatah AS bill_rs_trans, 
                      (CASE WHEN dbo.mt_master_tarif_detail.bill_dr1_ass IS NULL THEN 0 ELSE dbo.mt_master_tarif_detail.bill_dr1_ass END) AS bill_dr1, 
                      dbo.tc_trans_pelayanan.bill_dr1_jatah AS bill_dr1_trans, (CASE WHEN dbo.mt_master_tarif_detail.bill_dr2_ass IS NULL 
                      THEN 0 ELSE dbo.mt_master_tarif_detail.bill_dr2_ass END) AS bill_dr2, dbo.tc_trans_pelayanan.bill_dr2_jatah AS bill_dr2_trans, dbo.mt_master_tarif_detail.total, 
...
WHERE     (dbo.tc_trans_pelayanan.kode_tc_trans_kasir IS NULL OR
                      dbo.tc_trans_pelayanan.kode_tc_trans_kasir = 0) AND (dbo.tc_trans_pelayanan.kode_tarif NOT IN
                          (SELECT     kode_tarif
                            FROM          dbo.mt_master_tarif_detail_bedah)) AND (dbo.mt_master_tarif.flag_rujukan = 0 OR
                      dbo.mt_master_tarif.flag_rujukan IS NULL)
...
");
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [v_pindah_billing_assuransi]");
    }
};
