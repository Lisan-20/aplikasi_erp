...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.gd_daftar_antrian_pulang_erm_v
AS
SELECT     TOP (100) PERCENT dbo.mt_master_pasien.no_mr, dbo.tc_kunjungan.no_kunjungan, dbo.mt_master_pasien.nama_pasien, dbo.mt_master_pasien.almt_ttp_pasien, dbo.tc_kunjungan.no_registrasi, 
                      ...
                      dbo.gd_tc_gawat_darurat.status_periksa, dbo.tc_registrasi.stat_celaka, dbo.tc_registrasi.id_triase_identitas, dbo.tc_registrasi.tgl_ver, dbo.tc_registrasi.flag_ver, YEAR(dbo.tc_kunjungan.tgl_keluar) 
                      AS Expr1, MONTH(dbo.tc_kunjungan.tgl_keluar) AS Expr2, DAY(dbo.tc_kunjungan.tgl_keluar) AS Expr3, dbo.tc_registrasi.catatan_rm, dbo.tc_registrasi.id_lengkap_erm
...
WHERE     (dbo.tc_kunjungan.kode_bagian_tujuan LIKE '02%') AND (dbo.tc_kunjungan.status_batal IS NULL) AND (MONTH(dbo.tc_kunjungan.tgl_keluar) = MONTH(GETDATE())) AND 
                      (YEAR(dbo.tc_kunjungan.tgl_keluar) = YEAR(GETDATE())) AND (DAY(dbo.tc_kunjungan.tgl_keluar) = DAY(GETDATE())) AND (dbo.gd_tc_gawat_darurat.status_periksa = 1)
");
    }
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [gd_daftar_antrian_pulang_erm_v]");
    }
};
