...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.lap_rl3_gigi_v
AS
SELECT     dbo.mt_master_tarif.tingkatan, dbo.mt_master_tarif.kode_tarif, dbo.mt_master_tarif.kode_bagian, COUNT(dbo.tc_trans_pelayanan.nama_tindakan) AS jumlah, YEAR(dbo.tc_trans_kasir.tgl_jam) 
                      AS thn, dbo.mt_master_tarif.nama_tarif, MONTH(dbo.tc_trans_kasir.tgl_jam) AS bln, dbo.mt_master_tarif.referensi, dbo.tc_trans_kasir.status_batal, dbo.tc_trans_kasir.tgl_jam
...
GROUP BY dbo.mt_master_tarif.tingkatan, dbo.mt_master_tarif.kode_tarif, dbo.mt_master_tarif.kode_bagian, MONTH(dbo.tc_trans_kasir.tgl_jam), YEAR(dbo.tc_trans_kasir.tgl_jam), 
                      dbo.mt_master_tarif.nama_tarif, dbo.mt_master_tarif.referensi, dbo.tc_trans_kasir.status_batal, dbo.tc_trans_kasir.tgl_jam
HAVING      (dbo.mt_master_tarif.tingkatan > 3) AND (dbo.mt_master_tarif.referensi = 111010100) AND (dbo.tc_trans_kasir.status_batal IS NULL)
");
    }
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [lap_rl3_gigi_v]");
    }
};
