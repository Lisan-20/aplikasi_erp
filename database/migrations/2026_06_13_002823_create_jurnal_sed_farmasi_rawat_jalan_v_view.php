...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.jurnal_sed_farmasi_rawat_jalan_v
AS
...
WHERE     (dbo.mapping_transaksi_rs_v.kode_proses = 2) AND (dbo.tc_trans_kasir.seri_kuitansi = 'RJ') AND (dbo.tc_trans_pelayanan.kode_barang <> '' OR
                      dbo.tc_trans_pelayanan.kode_barang IS NOT NULL) AND (dbo.tc_trans_pelayanan.status_kredit = 0 OR
                      dbo.tc_trans_pelayanan.status_kredit IS NULL) AND (dbo.tc_trans_pelayanan.flag_jurnal = 0) AND (dbo.tc_trans_kasir.status_batal IS NULL) AND 
                      (dbo.mapping_transaksi_rs_v.acc_kredit > 0) AND (dbo.tc_trans_pelayanan.flag_jurnal IS NULL)
...
");
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [jurnal_sed_farmasi_rawat_jalan_v]");
    }
};
