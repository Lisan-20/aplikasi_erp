...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.v_trans_far_ass_RJ_retur
AS
...
WHERE     (dbo.tc_trans_kasir.status_batal IS NULL) AND (dbo.tc_trans_pelayanan.kode_kelompok = 3)
...
HAVING      (dbo.tc_trans_pelayanan.flag_jurnal = 0) AND (dbo.tc_trans_pelayanan.jenis_tindakan IN (11)) AND (dbo.tc_trans_kasir.seri_kuitansi IN ('AJ', 'RJ', 'NK')) AND 
                      (dbo.tc_registrasi.kode_perusahaan > 0) AND (dbo.tc_trans_pelayanan.status_kredit = 1) AND (dbo.mt_perusahaan.flag = 1)
");
    }
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [v_trans_far_ass_RJ_retur]");
    }
};
