<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        if (Schema::hasTable('dd_gaji_pokok')) {
            return;
        }

        Schema::create('dd_gaji_pokok', function (Blueprint $table) {
            $table->increments('id_dd_gapok');
            $table->integer('id_dd_tg')->nullable();
            $table->integer('id_dd_gg')->nullable();
            $table->decimal('gapok', 19, 4)->nullable();
            $table->string('status_gapok', 1)->nullable()->default('0');
            $table->integer('input_id')->nullable();
            $table->dateTime('input_tgl')->nullable();
            $table->smallInteger('status')->nullable();
            $table->dateTime('status_tgl')->nullable();
            $table->dateTime('tgl_berlaku')->nullable();
            $table->dateTime('tgl_berakhir')->nullable();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('dd_gaji_pokok');
    }
};
