...
return new class extends Migration
{
    ...
    {
        if (Schema::hasTable('transaksi_pj_umd')) {
            return;
        }

        Schema::create('transaksi_pj_umd', function (Blueprint $table) {
            $table->integer('id_trans_pj_umd');
            $table->integer('id_bd_tc_trans');
            $table->string('acc_no_1', 50);
            $table->string('acc_no_2', 50)->nullable();
            $table->tinyInteger('tx_tipe')->nullable();
            $table->decimal('jumlah_transaksi', 18);
            $table->decimal('jumlah_umd', 18);
            $table->decimal('selisih', 18, 0)->nullable();
            ...
            $table->primary(['id_trans_pj_umd'], 'pk_transaksi_pj_umd');
        });
    }
    ...
    {
        Schema::dropIfExists('transaksi_pj_umd');
    }
};
