...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.ri_tc_riwayat_pasien_masuk_v
AS
SELECT     dbo.mt_master_pasien.no_mr, dbo.tc_registrasi.no_registrasi, dbo.tc_kunjungan.no_kunjungan, dbo.ri_tc_rawatinap.kode_ri, dbo.mt_master_pasien.nama_pasien, 
                      ...
                      dbo.tc_registrasi.kode_kelompok, dbo.tc_registrasi.status_batal, dbo.tc_registrasi.tgl_jam_masuk, dbo.tc_kunjungan.tgl_keluar AS Expr1, YEAR(dbo.tc_kunjungan.tgl_masuk) AS tahun, 1 AS jumlah, 
                      MONTH(dbo.tc_kunjungan.tgl_masuk) AS bln, dbo.tc_registrasi.kode_bagian_masuk, dbo.mt_dokter_bagian.kd_bagian
FROM         dbo.tc_kunjungan INNER JOIN
                      ...
                      dbo.mt_dokter_bagian ON dbo.ri_tc_rawatinap.dr_merawat = dbo.mt_dokter_bagian.kode_dokter
WHERE     (dbo.tc_registrasi.status_batal IS NULL) AND (YEAR(dbo.tc_kunjungan.tgl_masuk) >= 2015)
");
    }
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [ri_tc_riwayat_pasien_masuk_v]");
    }
};
