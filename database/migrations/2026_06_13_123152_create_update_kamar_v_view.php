...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.update_kamar_v
AS
SELECT     TOP (200) dbo.tc_trans_pelayanan.kode_trans_pelayanan, dbo.tc_trans_pelayanan.kode_tc_trans_kasir, dbo.tc_trans_pelayanan.no_kunjungan, 
                      ...
                      dbo.tc_trans_pelayanan.flag_obat, dbo.tc_trans_pelayanan.jml_konversi, dbo.mt_master_tarif_ruangan.harga_r
FROM         dbo.tc_trans_pelayanan INNER JOIN
                      dbo.mt_master_tarif_ruangan ON dbo.tc_trans_pelayanan.kode_bagian = dbo.mt_master_tarif_ruangan.kode_bagian AND 
                      dbo.tc_trans_pelayanan.kode_klas = dbo.mt_master_tarif_ruangan.kode_klas AND 
                      dbo.tc_trans_pelayanan.no_bed = dbo.mt_master_tarif_ruangan.kode_ruangan
WHERE     (dbo.tc_trans_pelayanan.jenis_tindakan = 1) AND (dbo.tc_trans_pelayanan.nama_tindakan LIKE '%ruang%') AND 
                      (dbo.tc_trans_pelayanan.bill_rs_jatah = 0) AND (dbo.tc_trans_pelayanan.kode_bagian LIKE '03%') AND (dbo.tc_trans_pelayanan.kode_kelompok = 1) 
                      AND (dbo.tc_trans_pelayanan.status_selesai > 1) AND (dbo.tc_trans_pelayanan.nama_tindakan NOT LIKE '%ONE%') AND 
...
");
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [update_kamar_v]");
    }
};
