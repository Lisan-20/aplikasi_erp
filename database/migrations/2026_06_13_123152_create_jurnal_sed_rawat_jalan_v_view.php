...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.jurnal_sed_rawat_jalan_v
AS
SELECT     dbo.tc_trans_pelayanan.kode_tc_trans_kasir, dbo.tc_trans_pelayanan.no_registrasi, dbo.tc_trans_pelayanan.no_mr, dbo.tc_trans_pelayanan.kode_kelompok, 
                      ...
                      dbo.tc_trans_pelayanan.kode_bagian_asal, dbo.tc_trans_kasir.seri_kuitansi, SUM(dbo.tc_trans_pelayanan.lain_lain) AS Expr3, dbo.tc_trans_pelayanan.kode_barang, 
                      dbo.mapping_transaksi_rs_v.acc_debet, dbo.mapping_transaksi_rs_v.acc_kredit, dbo.mapping_transaksi_rs_v.nama_debet, 
...
WHERE     (dbo.mapping_transaksi_rs_v.kode_proses = 2) AND (dbo.tc_trans_kasir.seri_kuitansi IN ('RJ', 'AJ')) AND (dbo.tc_trans_pelayanan.kode_barang IS NULL) AND 
                      (dbo.tc_trans_pelayanan.flag_jurnal = 0) AND (dbo.tc_trans_kasir.status_batal IS NULL) AND (dbo.tc_registrasi.kode_kelompok = 1) AND 
                      (dbo.tc_registrasi.kode_perusahaan = 0 OR
                      dbo.tc_registrasi.kode_perusahaan IS NULL) AND (dbo.tc_trans_pelayanan.bill_dr1 = 0 OR
                      dbo.tc_trans_pelayanan.bill_dr1 IS NULL) AND (dbo.tc_trans_pelayanan.flag_jurnal IS NULL)
...
");
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [jurnal_sed_rawat_jalan_v]");
    }
};
