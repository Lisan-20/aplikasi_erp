...
return new class extends Migration
{
    ...
    {
        DB::statement("CREATE OR ALTER VIEW dbo.ri_cari_pasien_history_emr_v
AS
SELECT     dbo.mt_master_pasien.no_mr, dbo.tc_registrasi.no_registrasi, dbo.tc_kunjungan.no_kunjungan, dbo.ri_tc_rawatinap.kode_ri, dbo.mt_master_pasien.nama_pasien, 
                      ...
                      dbo.mt_master_pasien.mr_ibu, dbo.tc_kunjungan.tgl_kontrol, dbo.tc_registrasi.tgl_ver, dbo.tc_registrasi.flag_ver, DAY(dbo.tc_kunjungan.tgl_keluar) AS Expr1, MONTH(dbo.tc_kunjungan.tgl_keluar) 
                      AS Expr2, YEAR(dbo.tc_kunjungan.tgl_keluar) AS Expr3, dbo.tc_registrasi.catatan_rm, dbo.tc_registrasi.id_lengkap_erm
...
WHERE     (dbo.ri_tc_rawatinap.status_pulang = 1) AND (DAY(dbo.tc_kunjungan.tgl_keluar) = DAY(GETDATE())) AND (YEAR(dbo.tc_kunjungan.tgl_keluar) = YEAR(GETDATE())) AND 
                      (MONTH(dbo.tc_kunjungan.tgl_keluar) = MONTH(GETDATE()))
");
    }
    ...
    {
        DB::statement("DROP VIEW IF EXISTS [ri_cari_pasien_history_emr_v]");
    }
};
